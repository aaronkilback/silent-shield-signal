...
**Fleet build-or-stop-advertising — PAIRED with the Pillar-3 producer question (2026-07-29).** The fleet-dormant gap is one half of a single strategic question: *do we build engagement for the 42 advertised agents, or stop advertising capacity we don't route to?* Per operator ruling, this is **parked WITH the Pillar-3 producer question** (registry-hygiene-phantom-sweep.md — `agent_world_predictions` has no producer) **behind WO-LEARN-UNFREEZE**. Same class: advertised capability vs. actual product need. Decided **together in one honest-capability session**, informed by what the brief / augmented-analyst workflow actually consumes. Neither half is decided alone.

---

## PRODUCT BOUNDARY — Fortress does not route null-client public-safety broadcasts (RATIFIED 2026-07-31)

**Fortress alerts on client-scoped intelligence only.** A pageable alert with no `client_id` (a province-wide public-safety broadcast — e.g. an Alert Ready **Amber Alert** or a **provincial civil emergency**) has, by design, **no routing target**: recipients are resolved exclusively from `client_alert_recipients` keyed by `client_id` (`fetchVerifiedRecipientEmails` returns `[]` for a null client; `claim_pending_email_alerts` hard-requires `client_id IS NOT NULL`). This is **not a gap to close** — it is the boundary of the product.

- **Alert Ready and provincial/federal emergency systems own broadcast distribution.** Fortress does not, should not, and will not duplicate that channel. A July Amber Alert paged in August would be worse than not paged at all.
- **`refuse-to-emit` is the COMPLETE fix.** `resolveAlertEmission` correctly refuses to materialize a delivery-tier alert with no verified recipient (logs to `alert_emission_refusals`); a null-client broadcast falls into exactly this refusal and is the intended terminal state. There is nothing further to build.
- **Evidence (2026-07-31 read-only review):** every one of the 13,993 superseded-never-sent alerts, and all 10 recent pageable ones (2 `interruption` Amber/Civil-Emergency + 8 `notification`), carried a **null `client_id`**. The claim RPC never attempted them (`attempt_count=0`) because null-client rows are structurally unclaimable. The mechanism is working as designed.
- **If a client later wants broadcast AWARENESS**, it enters the platform as a **signal** (client-scoped intelligence the client is monitoring), **not as an alert** to broadcast. The client-scoped model is preserved end to end.
- **Revisit only if CRT or a future client explicitly asks** for broadcast awareness — and even then, as a signal-ingestion feature, not a broadcast-alert channel.
