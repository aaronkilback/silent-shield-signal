...
| Function state | **HARD-DISABLED 503 stub** since 2026-07-31 (v18, INC-AITOOLS-XTENANT-2026-07-30). Was a **deploy-drift orphan never in git** (`git log -S cron_heartbeat` on the fn path = empty), `verify_jwt=false`, gated only by a **compromised** `x-smoke-key`; it read every tenant bucket + could DELETE R2 objects. It has been a 503 **only since 2026-07-31** (NOT ~34 days — before that it ran daily and copied; see **CORRECTION 2026-08-11** below). |
...
## CORRECTION 2026-08-11 — the cadence DID run; the "monitored path" was the only thing missing
Direct R2 enumeration (read-only S3 listing of ss-fortress-dr, `scripts/dr-reconcile-r2.py`) **disproves the "cadence never ran / no daily incremental ever ran / everything since 2026-07-06 unprotected" conclusions above.** The bucket holds **24 daily system-briefing MP3s** (`tenant-files/_system/briefings/system/`), one per day at **08:23 UTC — exactly the `23 8 * * *` cron schedule — from 2026-07-07 through 2026-07-31**, then nothing. Nothing but `dr-storage-backup` writes to ss-fortress-dr (grep-confirmed), so:
- **The cadence RAN daily and copied the incremental delta (the briefing MP3, the one object that changed each day) for three weeks (07-07 → 07-31).** It did not "never run."
- **`cron_heartbeat` has 0 rows because the function never emitted a heartbeat — not because it never executed.** The DIAG (and the neural "last: never") inferred "never ran" from the ABSENCE of the heartbeat signal, without checking the artifact the function actually produced. The heartbeat path was broken; the copy path worked.
- **The real gap is 2026-07-31 → now (~11 days), since the containment 503 — not ~34 days, and not "nothing ever."** Storage changes from 07-07→07-31 WERE protected (the daily briefing was the only daily change); only 07-31→now is unprotected.
- **The lesson (recorded in the KB):** this DIAG trusted the missing heartbeat over the produced artifact and concluded "broken/never," when the artifact said "working." Ground truth is what the system produced (the R2 objects), not what it reported about itself (the heartbeat). See [[feedback_ground_truth_is_the_artifact_not_the_report]].

**Full reconciliation + the corrected premise:** `docs/platform-operations/backlog/WO-DR-CADENCE-REBUILD.md` (RECONCILIATION COMPLETE, 2026-08-11). WORM lock `snapshot-worm-90d` applied 2026-08-11 (retention until 2026-11-09).
...
### 3b. LOOSE END RESOLVED — real reasoning didn't "stall," the dominant ingestion path bypasses the scorer

The question: is `signal_agent_analyses = 0/24h` expected-low, or did `review-signal-agent` stall? **Answer: neither — the reasoning pipeline is starved upstream because ~84% of signals now enter via the RSS path, which never scores them.**

**Evidence:**
- **`signal_agent_analyses` per day (30d):** not never-frequent — real activity that **tapered**: 104 (Jul 17), 85 (Jul 18), 61 (Jul 22), 55 (Jul 23) → 11 (Jul 29) → 2 (Aug 1) → 8 (Aug 3) → **0 (Aug 4)**.
- **`composite_confidence` scored-rate per week (90d):** **collapsed** — 85.7% (May 4) · 76.0% (May 18) → 33% (Jun 1) → 3–5% (Jun–Jul) → **0.0% (this week, 0 of 51)**.
- **Tier-2 band in last 24h:** of 41 signals, **41 have `composite_confidence = NULL`** — 0 in the 0.60–0.75 band, 0 eligible (≥0.60). Nothing to dispatch.
- **By source (7d):** `rss` **273 signals / 0 scored (0%)**, `pattern` 29 / 0, vs `monitor-cisa-kev` 3 / 3 (100%) and misc `ingest-signal` 11/19. **The RSS path produces the overwhelming majority and scores none of them.**
- **Writers of `composite_confidence`:** `ingest-signal` (computeComposite), `ai-decision-engine`, `review-signal-agent` — **NOT `process-intelligence-document`** (the RSS path). So RSS signals are born unscored and never reach `ai-decision-engine`, whose fire-and-forget dispatch to `review-signal-agent` (line 1195: `if (isReviewableBand || isHighValueSignal || isHighSeverityLabel)`) is the only trigger.
- **`review-signal-agent` is not a cron** (0 in cron.job/registry/heartbeat) — purely event-driven from `ai-decision-engine`. No engine call → no review → no analysis.

**Causal chain:** RSS ingestion (`process-intelligence-document`, 84% of volume) writes signals with **NULL `composite_confidence`** and never calls `ai-decision-engine` → nothing reaches the tier-2 band → `review-signal-agent` never fires → **0 `signal_agent_analyses`** → neural "42 idle / dormant" + watchdog "9/42 ran." The taper tracks ingestion shifting from `ingest-signal` (scored) to the RSS path (unscored) over ~2.5 months.

**Connection to the 91-day "Tier-2 review gap: 0% reviewed" finding — CONFIRMED it's been telling us for three months.** With scored-rate at 2–10% since early June and 0% now, the tier-2 eligible pool has been starved for ~3 months. The finding is real and has been firing the whole time — but it read as *chronic* (0% forever, ambient) instead of *broken* (the dominant path bypasses the scorer + dispatcher). **This also indicts the watchdog's own fleet-dormancy ruling** (line 3620): it labels the dormancy "KNOWN STRATEGIC — capability configured beyond adoption, no same-day action." That is a **misdiagnosis** — the fleet isn't idle because it's over-configured; it's idle because 84% of its input never gets scored or routed. A fixable pipeline gap dressed as a roadmap decision. (Report only — the fix, when ruled, is upstream: score signals on the RSS path, or route RSS signals through `ai-decision-engine`.)

**Conclusion — the operator's core worry is correct, culprit inverted.** The self-reporting layer *is* unreliable, but not because "idle" is wrong — "idle/dormant/9-ran" are the honest, work-coupled numbers. The unreliability is that the **same screen also shows a synthetic-heartbeat panel ("Live Activity scanning", "96 scans", "54 alerts") with no label saying it's a liveness pulse, not real output.** An operator glancing at it reasonably concludes agents are working. The 2026-05-10 fix was **half-applied**: the *status* panel + watchdog were switched to the real table, but the *Live Activity* panel + scan counter still read the synthetic one. **Same class as the DR heartbeat gap and monitor-news-google "0 signals / 370 in DB": a displayed metric decoupled from the work it claims to represent.** The three findings share this root: *self-report ≠ work unless the write is coupled to the work.* (Report only — no fix applied; the genuine open question underneath is whether `signal_agent_analyses` being 0/24h is expected-low or `review-signal-agent` has itself stalled — a separate follow-on **resolved in §3b: starved, not stalled**.)

## 4. Rulings 2026-08-04 (record only — nothing starts today)

### 4a. `process-intelligence-document` implicated in FOUR defects — Phase 3 scope changed
The RSS path is one function starving pillar 2. Recorded as a Phase-3 requirement in **WO-GATE-KEYWORD-PRESCORE-01** ("Phase 3 REQUIREMENT (2)"): **RSS-path signals must receive `composite_confidence` and route through `ai-decision-engine` on the same terms as `ingest-signal`-path signals** — Phase 3 cannot just replace the matcher, or the semantic gate feeds agents that still never run. Four defects: keyword string-overlap match · `min_relevance_score` never read (WO-CLIENT-THRESHOLD-BYPASS-01) · asset-label collision (§2) · never writes `composite_confidence` (§3b). B-query re-run ~2026-08-05T19:36Z; Phase 3 follows.

### 4b. Watchdog fleet-dormancy ruling — RETRACTED, re-filed as defect
The system-watchdog fleet-dormancy finding (index.ts ~L3620) currently self-rules **"KNOWN STRATEGIC — capability configured beyond adoption, no same-day action" (low)**. **Operator retraction 2026-08-04:** that is a **misdiagnosis of a pipeline break as a roadmap decision.** When built (WO-WATCHDOG-FINDING-DISCIPLINE / Phase 3), **retract the "known-strategic" branch, re-file as a DEFECT**, and **cross-reference the 91-day tier-2 review gap** (same root: RSS path unscored → nothing eligible → fleet has no work). The dormancy is an *effect* of the unscored-RSS-path defect, not an independent strategic gap.

> **KB lesson (record):** *A finding that explains away a symptom is worse than one that shouts, because it stops the question being asked.* The "known strategic, no action" label converted a live pipeline break into ambient background for weeks. A watchdog's job is to keep the question open until the cause is found, not to pre-absolve a symptom with a plausible-sounding roadmap story. Sibling of the attention doctrine: a false "all fine / known" is as corrosive as a false alarm — it spends trust in the opposite direction.

### 4c. Build order (when the operator releases these — NOT today)
1. **(a)** Phase 2 B-query (~2026-08-05T19:36Z) → **Phase 3 gate rebuild INCLUDING scoring + dispatch on the RSS path** (WO-GATE-KEYWORD-PRESCORE-01 Phase 3 REQUIREMENT (2)).
2. **(b)** WO-SYNTHETIC-ACTIVITY-REMOVAL — small; stops the board lying while (a) is in flight.
3. **(c)** WO-WATCHDOG-FINDING-DISCIPLINE (includes the 4b retraction/re-file).
4. **(d)** WO-METRIC-PROVENANCE-AUDIT — full sweep.
5. **(e)** WO-DR-CADENCE-REBUILD.
All report-only until released.
