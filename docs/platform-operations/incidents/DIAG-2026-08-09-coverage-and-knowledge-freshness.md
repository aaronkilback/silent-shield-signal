...
**Correction logged (operator, 2026-08-09):** the earlier claim that PECL's residual was "generic direct keywords (`energy`/`pipeline`/`lng`)" was **wrong** — all 42 PECL keywords are specific. The operator did not accept the framing; the keyword list was checked, and the residual turned out to be the same broad tier-2 compounds already fixed by the anchor tighten (535/567) plus competitor names (29). **The defect was never in PECL's keyword list.** Recorded so the account is accurate about where the defect was (tier-2 anchors + competitor labeling), not where it was assumed to be.

### A.3 — BC Place decline = NEWS-CYCLE, not a matching gap (looked 2026-08-09)
Verdict: **news-cycle.** Evidence: (1) keywords are well-formed and matching — `bc place` (28/30d), `bc lions` (28), `vancouver whitecaps` (12), `bc place stadium` (8), `security incident` (11), World Cup terms; (2) **only 1** BC-Place-relevant article hit `no_client_match` in 7 days — a matching gap would show many; (3) `industry='venue_security'` = **event-driven, inherently spiky** coverage (peak 13/day on a late-July event day → lull early August); (4) the anchor tighten did NOT touch it (tier-2 fuzzy is energy-industry only). Daily shape: ~5/day late July → ~1/day early Aug → 0 on 08-08/09. **No action on matching.** Note: a rolling 30d→7d "decline" metric over-signals for event-driven venue clients — the metric is noisy by client type. Revisit only if 0 persists across an actual BC Place event window (Lions/Whitecaps home game, World Cup 2026 ramp).
