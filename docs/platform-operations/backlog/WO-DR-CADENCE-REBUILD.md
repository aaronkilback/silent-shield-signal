...
## R/W TOKEN VERIFICATION (2026-08-11) — the structural guarantee DEMONSTRATED

Short-TTL Object-R/W token (ss-fortress-dr only), staged on disk, tested via `scripts/dr-verify-rw-token.py`, creds removed after. All three pass:
- **TEST 1 (scope):** listing any other bucket → **AccessDenied**. The token is structurally confined to ss-fortress-dr.
- **TEST 3 (write):** PUT + read-back of the deliberate probe `incremental/_verify/2026-08-11-lock-test` (140 bytes).
- **TEST 2 (the guarantee — DEMONSTRATED, not inferred):** with the R/W token, DELETE and OVERWRITE were attempted against **the probe AND a real backup object** (the July-30 briefing MP3 — a regenerable, lowest-blast-radius subject). Every attempt raised **`ObjectLockedByBucketPolicy`** and read-back confirmed the object **survives, ETag unchanged.** (Note: R2's S3 API returns an explicit lock error here — unlike wrangler's earlier silent "delete complete" lie; ground truth was still taken from read-back regardless.)
- **VERDICT: the Object R/W token CANNOT delete or overwrite locked objects.** The function's own credential structurally cannot destroy the backup — the WORM lock holds against exactly the credential the function will carry.

### The deliberate probe object
`incremental/_verify/2026-08-11-lock-test` (140 bytes) was written to prove write + lock. It is **itself now under `snapshot-worm-90d` and CANNOT be removed until 2026-11-09** — this is by design (it demonstrates the lock protects new incrementals too) and is expected/deliberate, not a stray. It sits in the `incremental/` prefix where real incrementals will land. Leave it; it clears when the lock lapses (or is skipped at the next reconciliation).

## The Jul-6 Object R/W token — in secrets, RETIRE at rebuild (operator flag 2026-08-11)
The original DR function's credential (Object R/W, ss-fortress-dr, **created 2026-07-06, active, no expiry**, minted when the fn had `verify_jwt=false` + a compromised smoke key) **IS wired into Supabase secrets:** `R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY` / `R2_BUCKET` / `R2_ENDPOINT` (confirmed present via `supabase secrets list`; no repo code reads them — the orphan fn was never in git). 
- **Immediate risk is contained by the lock:** the Jul-6 token, like any Object R/W token, can no longer delete/overwrite existing objects (just proven) — it can only PUT new keys. So it can't destroy the backup. Retiring it is **credential hygiene** (remove a compromised-regime standing key), not an emergency.
- **Retire at rebuild — replace-then-revoke:** mint a FRESH Object R/W token (ss-fortress-dr only) → overwrite `R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY` secrets with it → deploy the rebuilt fn → verify it authenticates + writes an incremental → **then revoke the Jul-6 token in Cloudflare.** The fresh token gets an expiry + rotation reminder (unlike the Jul-6 no-expiry).
