...
### Rework 2 — made EXECUTABLE (Codex, 2026-09-16), in `WO-ATTRIBUTION-FIX-draft.sql`
- **Fix 1 is now enforced in SQL:** `subject_identity_anchors.source` is `NOT NULL CHECK (length(trim(source)) > 0)` — a missing-provenance anchor is rejected at write, and the resolver treats **unknown provenance as non-independent** (a strong anchor with no `source` cannot confirm anything).
- **Fix 3 is a real constraint, not a comment/trigger:** `chk_sei_finding_requires_confirmed_attribution` = `CHECK (NOT (is_finding AND attribution_state IN ('unattributed','unresolved','contradicted')))` on `subject_exposure_items` — non-bypassable at the storage layer; any classifier/UPDATE that tries to raise `is_finding` on an unresolved row fails loudly (added `NOT VALID`, validated after the §6 reclassification).
- **Fix 4 is executable code:** `attribution_gate_enabled()` (real function over the active-subject set) + a deploy-time `DO $$ … RAISE EXCEPTION … $$` that refuses to enable the gate while any active subject has zero verified anchors.
