...
Both gate on a `source_health_registry` row per producer (WO-COVERAGE substrate): `expected_daily_min`, `expected_daily_max`, `is_precision_feed` (declares legitimately-bursty/zero-most-windows).
...
> Producer has **0 terminal signals over its entire run history** despite **≥N lifetime successful runs**, AND `is_precision_feed = false` (i.e. it is declared as a producer expected to yield). → finding. Catches **court-registry (806 runs / 0 ever)** and the **five orchestrator monitors** once P1 gives them run records. **Does NOT fire on darkweb (498 runs / 0)** — darkweb is declared `is_precision_feed = true` (correctly quiet; verified 2026-08-14). The registry declaration is the whole discriminator between broken-silent and legitimately-sparse.
