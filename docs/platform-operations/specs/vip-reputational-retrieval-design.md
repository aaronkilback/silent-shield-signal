...
**Bottom line:** a thorough two-phase reputational scan costs **~$1–2 in retrieval**; COGS is not the constraint on a $10k product. The constraints are the daily CSE cap (throughput) and verification/clustering quality (accuracy). Acceptance test unchanged: given "Aaron Kilback", the scan must surface wiselaw.blogspot.com's Olynyk v. Kilback post AND cluster its known echoes (christopherdiarmani) into one exposure item — nothing ships until it does.
