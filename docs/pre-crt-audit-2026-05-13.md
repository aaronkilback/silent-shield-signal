...
| F-022 | SERIOUS | reproducibility | Production has schema artifacts NOT in migrations. Discovered during staging stand-up: `get_user_accessible_client_ids()` function (used by every tenant-scoped RLS policy) was created via SQL editor, never tracked in a migration file. A fresh prod rebuild from the migrations directory would have broken RLS silently. Plus duplicate-versioned migrations (two files named `20260503000002_*`), several data-seed migrations hardcoding production-only UUIDs, and at least one migration referencing a table not yet created. **2026-05-13 Phase 0 verification note (accepted temporary limitation):** the `detect_schema_drift()` watchdog shipped to staging covers tables + functions but not columns. Phase 0 synthetic-column test → `drift=false` (gap confirmed); synthetic-table test → `drift=true` (works). Column-level tracking deferred to Tier 0.1 prod backport so it ships as one coherent change set with one staging-verification pass before prod promotion. Tier 0.1 does not clear until column-level synthetic detection passes on staging. |
| F-023 | BLOCKER | tenancy / AI surface | **Support-chat as privileged attack surface.** Support-chat uses `SUPABASE_SERVICE_ROLE_KEY` and bypasses RLS for signal-lookup features. A Tenant A user asking "look up SIG-2026-XXXXXX" where the signal belongs to Tenant B receives the cross-tenant content directly. Discovered in third-pass review 2026-05-13 — same lookup-by-number feature shipped today is the leak vector. Mitigations: filter lookups by `get_user_accessible_client_ids()`; prompt-injection defense in system prompt; `support_chat_lookups` audit table; test pattern #6 in tenant isolation suite. |
