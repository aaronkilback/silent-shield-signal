// Canonical active-incident definition — EDGE mirror of the public.active_incidents
// view + public.is_incident_active() SQL function (migration
// 20260730120000_canonical_active_incidents_view.sql). Frontend mirror:
// src/lib/incident-status.ts. Keep all three in lockstep.
//
...
//
// USE:
//   • Server-side `.from()` reads of "active/open incidents" → query the
//     `active_incidents` VIEW, not `incidents` with a homemade status filter.
//   • In-memory filters over already-fetched rows → isIncidentActive(row).
//   • Status-only checks → ACTIVE_INCIDENT_STATUSES.has(status) / !isTerminalIncidentStatus(status).
...
// Active statuses, for building `.in('status', [...])` predicates where a view
// read is not convenient. Derived from the enum minus the terminal set:
// open, acknowledged, contained, investigating, mitigated.
...
// not superseded, not a test row. Pass the incident row.
