...
//   - It runs at temperature 0 as a separate call from derivation, so it is not literally the same generation.
...
export function makeLlmEntailmentJudge(model = "gpt-4o-mini"): EntailmentJudge {
  return async (inferenceText: string, overClaimTexts: string[]): Promise<EntailmentVerdict> => {
    ...
    const { data, error } = await callAiGatewayJson<{ entailed?: boolean; reason?: string }>({
      ...
      context: { phase: "grounding-entailment" },
