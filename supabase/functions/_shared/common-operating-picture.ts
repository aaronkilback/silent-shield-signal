...
export async function buildCOP(supabase: SupabaseClient, tenantId: string | null | undefined): Promise<COPSnapshot> {
  ...
  // NOTE: this is orthogonal to the temporal-integrity buckets below — counts
  // exclude projections; temporal annotation still labels recency of what's shown.
  ...
  // Temporal annotation (below) is applied to this filtered list, preserving the
  // temporal-integrity buckets for the items actually shown.
