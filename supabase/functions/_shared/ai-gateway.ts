...
function getProviderConfig(model: string): ProviderConfig {
  ...
  // Perplexity Sonar is a LIVE, web-grounded model. NEVER substitute an ungrounded model
  // (gpt-4o-mini / Gemini) when PERPLEXITY_API_KEY is unset — a non-searching model answers
  // real-time questions from training data and fabricates "current" facts (model-as-live-data
  // doctrine, 2026-08-30). Sonar always routes to Perplexity; with no key the request FAILS
  // CLOSED at the Perplexity endpoint (auth error surfaced as {error}), never silently
  // downgraded to a fabricating model. A caller that needs fresh search without Perplexity must
  // explicitly use the native Gemini google_search endpoint — the gateway will not fake it.
