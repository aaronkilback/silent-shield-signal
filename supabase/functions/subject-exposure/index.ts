...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: items } = await excludeSupersededExposure(supabase.from("subject_exposure_items")
      ...
      // WO-LEGAL-FABRICATION-CONTAIN (2026-08-26): legal category fabricates lawsuits + mislabels noise.
      // OFF entirely (not displayed here, not in any report) until the classifier is fixed. See
      // generate-subject-exposure-report for the full rationale. A blank section is fine; a fake lawsuit is not.
      .neq("category", "legal");
