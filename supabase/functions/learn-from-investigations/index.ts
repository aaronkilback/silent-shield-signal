...
/**
 * Learn from Investigations — Passive pattern extraction from closed/completed investigations.
 * 
 * Extracts:
 * 1. Synopsis structures and common phrasing
 * 2. Investigation workflow patterns (entry counts, timelines)
 * 3. Entity type associations
 * 4. Template candidates for future investigations
 * 5. Keyword/topic clustering for duplicate detection
 * 
 * Can be triggered manually or via cron (e.g., weekly).
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1. Fetch completed investigations with entries
    const query = supabase
      .from('investigations')
      .select(`
        id, file_number, synopsis, information, recommendations, 
        file_status, created_at, updated_at, client_id,
        correlated_entity_ids, cross_references, incident_id
      `)
    ...
    if (!investigations?.length) {
      console.log('[learn-from-investigations] No completed investigations to learn from');
    ...
    // 2. Fetch entries for these investigations
    ...
    // 3. Fetch entity types for correlated entities
    ...
    if (allEntityIds.length > 0) {
      const { data: entities } = await supabase
        .from('entities')
        .select('id, type')
        .in('id', allEntityIds.slice(0, 200));
    ...
    // Build a summary of all investigations for pattern extraction
    const investigationSummaries = investigations.map(inv => {
      const entries = entriesByInv.get(inv.id) || [];
      const entityTypes = (inv.correlated_entity_ids || [])
        .map((id: string) => entityTypeMap.get(id))
        .filter(Boolean);
      ...
      return {
        ...
        has_incident: !!inv.incident_id,
        synopsis_length: inv.synopsis?.length || 0,
        synopsis_preview: inv.synopsis?.substring(0, 300) || '',
        information_length: inv.information?.length || 0,
        recommendations: inv.recommendations?.substring(0, 200) || '',
        entry_count: entries.length,
        entity_types: [...new Set(entityTypes)],
        days_open: daysOpen,
        ai_entries: aiEntries.length,
        ai_accepted: acceptedAi,
        ai_rejected: rejectedAi,
...
1. "templates": Array of investigation templates (max 5). Each template has:
   - "template_name": short descriptive name
   - "category": one of [general, incident_response, vip_protection, travel_security, insider_threat, cyber, physical_security, fraud]
   - "description": 1-sentence description
   - "typical_synopsis_structure": a skeleton synopsis analysts can fill in
   - "typical_recommendations": array of 3-5 common recommendation patterns
   - "common_entity_types": entity types typically involved
   - "avg_entry_count": typical number of entries
   - "avg_days_to_close": typical days to resolution

2. "entry_patterns": Array of common entry writing patterns (max 10). Each has:
   - "pattern_name": short name
   - "pattern_template": a fill-in-the-blank entry template

3. "workflow_insights": Object with:
   - "avg_synopsis_length": typical synopsis word count
   - "most_common_entity_types": most frequently involved entity types
   - "ai_acceptance_rate": what percentage of AI suggestions were accepted
   - "workflow_bottlenecks": any patterns suggesting workflow friction
...
Return ONLY valid JSON, no markdown.`;
    ...
    if (!response.ok) {
      ...
      console.error('[learn-from-investigations] AI error:', response.status, errText);
    ...
    for (const template of (patterns.templates || [])) {
      const { error: tmplError } = await supabase
        ...
        .upsert({
          ...
          typical_synopsis_structure: template.typical_synopsis_structure,
          ...
          avg_entry_count: template.avg_entry_count || null,
          avg_days_to_close: template.avg_days_to_close || null,
          derived_from_count: investigations.length,
          derived_from_ids: invIds.slice(0, 20),
          client_id: clientId,
          confidence_score: Math.min(investigations.length / 10, 1.0),
        }, {
    ...
    // 6. Store workflow insights in learning_profiles
    ...
    await supabase.from('learning_profiles').upsert({
      ...
      features: {
        ...workflowInsights,
        entry_patterns: patterns.entry_patterns || [],
        templates_count: templatesStored,
        source_investigation_count: investigations.length,
        last_extraction: new Date().toISOString(),
      },
      sample_count: investigations.length,
      last_updated: new Date().toISOString(),
    }, {
      onConflict: 'profile_type',
    });

    // 7. Build investigation keyword index for duplicate detection
    ...
    for (const inv of investigations) {
      ...
      keywordIndex[inv.id] = uniqueWords;
    }
    ...
    await supabase.from('learning_profiles').upsert({
      ...
      sample_count: Object.keys(keywordIndex).length,
      last_updated: new Date().toISOString(),
    }, {
      onConflict: 'profile_type',
    });
    ...
    await supabase.from('agent_learning_sessions').insert({
      ...
      learnings: {
        templates_extracted: templatesStored,
        entry_patterns: (patterns.entry_patterns || []).length,
        workflow_insights: workflowInsights,
        source_count: investigations.length,
      },
      source_count: investigations.length,
      quality_score: Math.min(investigations.length / 5, 1.0),
    });

    console.log(`[learn-from-investigations] Complete. ${templatesStored} templates stored, workflow insights updated.`);
