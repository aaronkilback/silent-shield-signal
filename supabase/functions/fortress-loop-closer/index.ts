...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════════
    // LOOP 5: (REMOVED) AEGIS BRIEFINGS
    // Previously inserted a synthetic "AEGIS Fortress daily briefing"
    // message into ai_assistant_messages to keep a 24h-count watchdog
    // green. The briefings had no analyst value and polluted the home
    // page chat history with rows like "X signals ingested in the last
    // 24h, all intelligence loops are active". Removed at operator
    // request — if a 24h-count metric is needed, derive it from real
    // ingestion telemetry instead of fake content.
    results.aegis_briefings = { skipped: 'disabled — synthetic briefings removed' };
