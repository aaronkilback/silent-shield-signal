-- Fix trigger function to handle TEXT[] array correctly
...
DECLARE
  ...
  matches BOOLEAN;
BEGIN
  -- Skip if already processed (check array properly for TEXT[])
  IF NEW.applied_rules IS NOT NULL AND cardinality(NEW.applied_rules) > 0 THEN
    RETURN NEW;
  END IF;
  ...
  -- Update signal with rule results (check cardinality for TEXT[])
  IF cardinality(matched_rules) > 0 THEN
