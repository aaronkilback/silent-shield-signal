-- Fix trigger function to handle JSONB array for applied_rules
...
DECLARE
  ...
  matches BOOLEAN;
BEGIN
  -- Skip if already processed (applied_rules is JSONB)
  IF NEW.applied_rules IS NOT NULL AND jsonb_array_length(NEW.applied_rules) > 0 THEN
    RETURN NEW;
  END IF;
  ...
  -- Update signal with rule results (convert TEXT[] to JSONB for applied_rules)
  IF array_length(matched_rules, 1) > 0 THEN
    NEW.applied_rules := to_jsonb(matched_rules);
