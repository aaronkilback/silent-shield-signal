-- Schedule neural / AI-loop functions introduced in the speculative-dispatch wave.
-- Uses the same pg_cron + net.http_post pattern as existing schedules.

-- compute-signal-baselines — every 6 hours
...
-- self-improvement-orchestrator — nightly at 3 AM UTC
SELECT cron.schedule(
  'self-improvement-orchestrator-nightly',
  '0 3 * * *',
  $$
  SELECT net.http_post(
...
-- semantic-embed-knowledge — every 4 hours
SELECT cron.schedule(
...
-- calibration-updater — every 12 hours
SELECT cron.schedule(
...
-- propagate-knowledge-edges — every 2 hours
SELECT cron.schedule(
...
-- Register all jobs in the watchdog cron_job_registry
INSERT INTO public.cron_job_registry (job_name, expected_interval_minutes, description, is_critical)
VALUES
  ('compute-signal-baselines-6h',        360,  'Recomputes signal frequency/severity baselines for anomaly detection', true),
  ('self-improvement-orchestrator-nightly', 1440, 'Nightly AI self-improvement cycle — calibration, prompt updates, learning triggers', false),
  ('semantic-embed-knowledge-4h',        240,  'Embeds new expert knowledge and agent profiles into pgvector for semantic search', false),
  ('calibration-updater-12h',            720,  'Scores resolved debate predictions with Brier scores and updates agent calibration', false),
  ('propagate-knowledge-edges-2h',       120,  'Propagates significant agent learnings to semantically related agents', false)
