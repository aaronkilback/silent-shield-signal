...
3. **Staging cannot prove sibling-client isolation today.** The last-known staging topology has one observed user, one tenant, and one client. There is no Client A / Client B fixture with which to demonstrate the allowed and denied paths.
4. **The governed migration preflight did not apply anything.** From source commit `8edf95be120dde886a3954d4f9fcd49352fb41c9`, the preflight proved the staging target and approved migration hash, then failed closed during `migration_history_read` after 60 seconds with termination confirmed. Remote migration history was not obtained and no apply command ran.
...
6. **RLS Advisor warnings are separate inventory work.** Warnings for public-schema tables with RLS disabled were observed. Their exposure, use, and remediation have not been classified; no blanket enable-RLS action is authorized.
7. **The deployment relationship remains unresolved.** Staging and production are not assumed to mirror one another. The Worker/Pages/deployment-trigger relationship is not yet sufficiently proven to treat a merge to `main` as a routine frontend release.
...
**Next gate:** Support/status evidence must justify one explicitly authorized, staging-only Evidence Operation. The packet must name one read-only command/path, this exact target, one receipt location, and one stop condition. A successful preflight would still not authorize apply.
