...
### Anon-surface enforcement (apex-audit 2026-08-02) — "watchdog proves the door is shut; WRAITH detects whether anyone came through"

The apex `silentshieldsecurity.com` ships **Fortress prod's own anon key** in a public bundle (the anon key is public-by-design — it's also in the Fortress app bundle). So the entire anon-reachable surface of `kpuqukppbmwebiptqmog` is exposed to the internet, and **RLS-at-Creation is the load-bearing control**. Root cause of drift: anon holds INSERT/UPDATE/DELETE grants on ~394 public tables by Supabase default — every table is anon-writable *unless RLS denies*. Findings closed 2026-08-02: 5 INSERT policies mis-scoped `TO public` (re-scoped `TO service_role`); 22 SECURITY DEFINER auth-graph oracles anon-EXECUTE-able (`has_role`, `is_super_admin`, `get_user_*`, `is_tenant_member`, `registry_phantom_check`, …) revoked from anon (authenticated+service_role retained for RLS eval); 3 storage SELECT policies re-scoped off anon.

- **Deterministic guard = `agent-sentinel` Probe 2f** → calls RPC **`public.security_anon_surface_scan()`**, fires `high` per non-empty dangerous set: (1) RLS-disabled public tables, (2) anon/public unconditional write policies, (3) anon/public `USING(true)` read policies, (4) anon-EXECUTE SECURITY DEFINER functions, (5) anon-read storage policies / public buckets. (6) RLS-on-no-policy is a `low` informational decision surface.
- **Allowlists are DATA, not code** — `public.security_anon_surface_allowlist` (invariant, identifier, reason). An intentional exception is a reviewed INSERT, never a probe edit. Seeded: `spatial_ref_sys` (RLS-off PostGIS), `st_estimatedextent`×3 (PostGIS), and the auth.uid()-scoped no-arg variants `get_user_accessible_client_ids()` / `operator_invite_membership_check()` (return `[]` to anon).
- **Behavioral backstop = WRAITH** (`docs/platform-operations/backlog/WO-WRAITH-PIPELINE-INJECTION-DETECTION.md`): detects unattributed / provenance-less pipeline writes → injection log → source-credibility loop. Watchdog can only catch a misconfig *after* it exists; WRAITH detects exploitation during any open window.
- **Marketing decoupling** (`docs/platform-operations/backlog/WO-DECOUPLE-MARKETING-FROM-FORTRESS-PROD.md`) is hygiene, not remediation — the public marketing site should not carry a Fortress-prod credential at all, but decoupling fixes none of the above (the anon key is public on the Fortress app regardless).
...
> **Lockstep duplication (tech debt):** the short-keyword detection — `SHORT_KW_ALLOWLIST` (seeded `{'lng'}`) + strip (`replace(/^(asset|keyword|kw|tier2|tier-2):/,'')`) + anchor rule (fabricated iff every matched keyword is ≤5 chars AND not allowlisted) — is duplicated in `process-intelligence-document` (born-quarantine) and `agent-sentinel` Probe 2d. **The allowlist AND the logic MUST stay identical in both** or the probe and write-path disagree (a real short acronym gets silently born-quarantined). The `lng` allowlist entry prevents legitimate PECL `LNG` matches from being suppressed. Extract to `_shared/` when convenient (same pattern as `_shared/safe-fetch.ts`). The whole length-heuristic + allowlist is **transitional** — it retires once the Phase-3 semantic matcher anchors on token boundaries (`home` in `homelessness` = boundary violation; `LNG` in `LNG Canada` = whole-token). See WO-GATE-KEYWORD-PRESCORE-01 + WO-CLIENT-THRESHOLD-BYPASS-01.
