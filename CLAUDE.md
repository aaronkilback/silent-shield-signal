...
- **Migrations are the same rule one layer down (2026-08-31 amendment).** A migration is done when its EFFECT is verified in prod — the table exists, the column is there, the function body contains the change, the grant/revoke is in force — **not when the migration file exists in the repo**. The repo migration ledger is NOT a reliable index of prod state: `apply_migration` records a generated version/name, so a file's presence, and even its name-match in `supabase_migrations.schema_migrations`, proves nothing about whether its content ran. **Verify migrations by CONTENT probe** (`to_regclass`, `information_schema.columns`, `pg_get_functiondef`/`prosrc`, `pg_policies`, `has_table_privilege`, `cron.job`), never by ledger membership. **Provenance:** WO-MIGRATION-LEDGER-SWEEP found 83 repo names absent from the ledger — but content-probing showed the vast majority were applied under a divergent identifier (identifier-divergence, per WO-LEDGER-RECONCILE), while a genuinely-unapplied handful — including `auto_approve_null_join_fix`, a written-but-never-shipped severity-queue fix — hid among them. Ledger-absence ≠ unapplied; ledger-presence ≠ applied. Only the effect is truth.
